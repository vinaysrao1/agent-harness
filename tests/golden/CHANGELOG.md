...
| 2026-08-22 | S-003 | `f767e4a` + S-003 | `replay_corpus.jsonl`, `replay_trace.json` (initial freeze) | Corpus derived from the local `jobs/` tree as of this date — 25 trials selected from 645, heaviest-first plus a spread. **Provenance is not fully recoverable**: `jobs/` is gitignored, so the exact runs behind these shapes cannot be reconstructed from the repository. Recorded as a known limitation rather than left implicit. No behavior change; Lane A. |
